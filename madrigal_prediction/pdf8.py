...
def addCount(C, mxlname, triplets):
	...
	while cur:
		...
		#alto1=int(btwn[ALTO])
		triplets[getKey({CHORD:chord, ALTO:alto})] = [chord, alto]
		...
		# ALTO
		child = abs(int(cur[ALTO]))
		parents = {ALTO:abs(int(prev[ALTO])), CHORD:cur[CHORD], SOPRANO:cur[SOPRANO]}
		addToDict(D[ALTO], child, parents)

		# TENOR
		child = abs(int(cur[TENOR]))
		parents = {TENOR:abs(int(prev[TENOR])), CHORD:cur[CHORD]}
		addToDict(D[TENOR], child, parents)
