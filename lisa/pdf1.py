...
def addCount(C, mxlname, triplets):
	...
	while cur:
	    ...
	    # SOPRANO
		child = abs(int(cur[SOPRANO]))
		parents = {SOPRANO:abs(int(prev[SOPRANO])), CHORD:chord}
		addToDict(D[SOPRANO], child, parents)

		# ALTO
		child = abs(int(cur[ALTO]))
		#parents = {ALTO:abs(int(prev[ALTO])), SOPRANO:abs(int(cur[SOPRANO])), CHORD:chord}
		parents = {SOPRANO:abs(int(cur[SOPRANO])), CHORD:chord}
		addToDict(D[ALTO], child, parents)
		...
		if child < 0:
			child = -1
		parents = {SOPRANO1:abs(int(prev[SOPRANO])), SOPRANO:abs(int(cur[SOPRANO]))}
...
def printXML(mxlname):
	...
	while cur:
		print my_offset, abs(int(cur[SOPRANO])), abs(int(cur[ALTO])), int(cur[TENOR]), int(cur[BASS]), cur[CHORD]
		my_offset += .5
		cur = times[my_offset]
...
#for note in ALTO_RANGE:
#	print p[ALTO](note, {SOPRANO:60})

#for note in SOPRANO_RANGE:
#	print p[SOPRANO](note, {SOPRANO:60, TIME:3})
