...
def addCount(C, mxlname, triplets):
	...
	parents = {SOPRANO:abs(int(cur[SOPRANO])), TENOR:abs(int(cur[TENOR])), BASS:abs(int(cur[BASS]))}
	...
	while cur:
		...
		alto1=int(btwn[ALTO])
		triplets[getKey({CHORD:chord, ALTO:alto, ALTO1:alto1})] = [alto, alto1]
		#print [chord, alto, alto1]

	    # SOPRANO
		child = abs(int(cur[SOPRANO]))
		parents = {SOPRANO:abs(abs(int(prev[SOPRANO])))}
		addToDict(D[SOPRANO], child, parents)

		# ALTO
		child = abs(int(cur[ALTO]))
		#parents = {ALTO:abs(int(prev[ALTO])), CHORD:cur[CHORD], SOPRANO:abs(int(cur[SOPRANO]))} # with links btwn alto
		parents = {SOPRANO:abs(int(cur[SOPRANO])), TENOR:abs(int(cur[TENOR])), BASS:abs(int(cur[BASS]))} # without links btwn alto
		addToDict(D[ALTO], child, parents)

		# TENOR
		child = abs(int(cur[TENOR]))
		parents = {TENOR:abs(int(prev[TENOR]))}
		addToDict(D[TENOR], child, parents)

		# BASS
		child = abs(int(cur[BASS]))
		parents = {BASS:abs(int(prev[BASS]))}
		addToDict(D[BASS], child, parents)
